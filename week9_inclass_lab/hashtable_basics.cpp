...
struct HashTable{
    ...
    int hashfunc(string user_name){
        ...
        int hash = 0;
	//add your code below
	// check the ascii values of all the characters and add them
	...
	//add the pw if the key is empty only
        if (empty){
            password[hash]=user_password;
        }
        else{
            cout<<"The user_name cannot be inserted"<<endl;
        }
	...
	//add an if condition to complete the code here
        if (empty){
            cout<<"The user_name doesn't exist"<<endl;
        }
        else{
